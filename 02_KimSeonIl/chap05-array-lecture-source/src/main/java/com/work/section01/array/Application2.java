...
public class Application2 {
  public static void main(String[] args) {
    ...
    /* JVM 메모리 영역중 heap은 비어있는 상태로 존재할 수 없다!!
     * 할당 시 각 자료형의 기본 값으로 초기화 되어있다.
     *
     * 자료형별 기본값
     * - 정수/실수 -> 0, 0L ,0.0f, 0.0
     * - char -> '\000' (유니코드 0번) nul
     * - boolean -> false
     * - 참조형(ex. String) -> null
     * */
    ...
    /* hashCode() : 객체 (배열)의 주소 값을 10진수로 변환하여 반환하는 생성한 객체(배열)의 고유한 고유한 정수 값을 반환하는 메서드
     * */
    ...
    System.out.println("수정 : iarr = " + iarr);

    /* DB의 NULL :  빈칸(저장된 데이터가 없다)
     *  Java의 null : 저장된 주소가 없다(참조하는 객체가 없다)*/

    //iarr 에 아무 배열도 참조하지 않는다
    iarr = null;
    /*java.lan.NullPointerException: 아무것도 참조하지 않는 변수를 이용할때 발생하는 예외(에러)*/
    System.out.println("iarr.length = " + iarr.length);
    System.out.println("iarr.hashCode() : " + iarr.hashCode());
  }
}