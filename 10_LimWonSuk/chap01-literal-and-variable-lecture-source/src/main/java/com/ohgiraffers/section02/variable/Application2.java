...
public class Application2 {
 ...
 //int long; 에러
   ...
   /* java & 개발자들의 약속(규약)*/
   // 2-1. 변수명의 길이 제한은 없다.
   int wesdfsghaesgrhdftatgre = 10;
   System.out.println("wesdfsghaesgrhdftatgre = " + wesdfsghaesgrhdftatgre);
   ...
   * - 모든 단어를 붙여 쓰되, 연결되는 단어의 첫 글자를 대문자로 표현*/
   int maxAge = 40;
    ...
    String myNameIsBaekDongHyen = "백동현";

        /* 2-3. 단어와 단어 사이를 연결할 때 _(언더스코어를) 사용하지 않는다
        Snake-CAse : 연결되는 단어 사이를 _로 연결
        -> 다음 언어 또는 DB와 연결 시 모호함 발생시킬 수 있음
        * */
...
int s = 100; // 변수 선언과 동시에 초기화
   ...
   * */
   int sum = 0;
   int max = Integer.MAX_VALUE; // int 최대 값
   int min = Integer.MIN_VALUE; // int 최소 값
   double avg = 2.345;

   /* 2-7 변수명은 명사로 작성할 수 있도록 한다
   * */
   String goHome; // 동사 권잘 X, 동사는 method 이름으로 권장
   String Home;
